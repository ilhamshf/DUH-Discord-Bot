...
class MiyakoClient extends Client {
  ...
  onReady() {
    this.ready = true;
    this.console.log(`Menggunakan akun ${this.user.tag}`);
  ...
  async init() {
    ...
    this.console.log(`Memuat ${commands} perintah.`);
    this.console.log(`Memuat ${events} events.`);
    this.console.log(`Memuat ${monitors} monitors.`);
    ...
    this.console.log("Tersambung ke database");
    ...
    for (const [name, settings] of Object.entries(this.settings)) {
      await settings.init();
      this.console.log(`Memuat ${settings.cache.size} ${name}`);
    }
  }
}
