...
class Command extends Base {
  ...
  async verifyUser(msg, user, defaultToAuthor = false) {
    ...
    if (!match) throw "Duh! That's not a user mention or an ID. What were you trying to do?";
  ...
  async verifyMember(msg, member, defaultToAuthor = false) {
    ...
    if (!guildMember) throw "Duh! That user is not in this server.";
    return guildMember;
  }
  ...
  verifyRole(msg, rolename, optional = false) {
    ...
    if (!rolename) throw "Duh! You must provide a role name or ID.";
  ...
  verifyInt(num, def) {
    ...
    if (isNaN(parsed)) throw "Duh! You must provide a valid number.";
    return parsed;
  }
