...
class Warn extends Command {
  ...
  async run(msg, [user, ...reason]) {
    ...
    if(!reason.length) return msg.send("Duh! You didn't give me a reason.");

    if(user.id === msg.author.id) return msg.send("Duh! You can't warn yourself.");
    if(user.id === this.client.user.id) return msg.send("Why would you try to warn me?");
    if(user.bot) return msg.send("Duh! You can't warn bots.");
    if(user.id === msg.guild.ownerID) return msg.send("Duh! You can't warn the owner.");
