...
class Ban extends Command {
  ...
  async run(msg, [member, ...reason]) {
    ...
    if(member.id === msg.author.id) return msg.send("Duh! Why would you ban yourself?");
    if(member.id === this.client.user.id) return msg.send("Duh! Why would you ban me?");
    if(member.id === msg.guild.ownerID) return msg.send("Duh! You can't ban the owner.");
