...
class Load extends Command {
  ...
  async run(msg, [store]) {
    ...
    if(!(store instanceof Store)) return msg.send("Duh! That's not an actual store.");

    try {
      const before = store.size;
      await store.loadFiles();
      const after = store.size - before;
      return msg.send(`Berhasil reloaded/loaded ${store.size} ${store.name}. ${after === 0 ? "There was nothing new right?" : `${after} new ${store.name} were loaded.`}`);
    } catch(err) {
