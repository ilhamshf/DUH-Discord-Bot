...
class Poll extends Command {
  ...
  async run(msg, [channeltxt, ...choices]) {
    ...
    if(!args.length) return msg.send(`Duh! You need to provide me arguments. Usage: \`${msg.guild.settings.prefix}${this.usage}\``);

    // if(!channel.postable) throw "I cannot send message in that channel.";
    if(!channel.permissionsFor(msg.member).has("SEND_MESSAGES")) return msg.send("Duh! You can't post in that channel.");
