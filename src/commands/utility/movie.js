...
class Movie extends Command {
  constructor(...args) {
    super(...args, {
      ...
      extendedHelp: "e.g. `d!movie infinity war, 2`",
  ...
  async run(msg, args) {
    if (!args.length) return msg.send("Duh! What am I supposed to search?");

    const [query, page = 1] = args.join(" ").split(",");
    if (isNaN(parseInt(page))) return msg.send("Duh! Page must be a number.");
