...
class UserPrefix extends Command {
  ...
  async add(msg, args) {
    if(msg.author.settings.prefix && msg.author.settings.prefix.length >= 10)
      return msg.send("Duh! You can't have more than 10 prefixes. Remove some before trying again.");
    
    const prefixInput = args.join(" ").toLowerCase();
    if(!prefixInput) return msg.send("Duh! You must provide a prefix.");

    // User prefixes get an extra 5 chars compared to guild prefixes.
    if(prefixInput.length > 15) return msg.send("Duh! Prefix cannot be longer than 15 characters!");

    // Get existing prefixes to append to.
    const prefix = msg.author.settings.prefix || [];

    // Avoid duplicates.
    if(prefix.includes(prefixInput)) return msg.send("Duh! That prefix is already on the list.");

    prefix.push(prefixInput);

    await msg.author.update({ prefix });
    return msg.send(`Berhasil added the prefix \`${prefixInput}\` to your list of prefixes.`);
  }
  ...
  async remove(msg, args) {
    ...
    const prefixInput = args.join(" ").toLowerCase();
    if(!prefixInput) return msg.send("Duh! You must provide a prefix to remove!");

    const prefix = msg.author.settings.prefix;
    if(!prefix.includes(prefixInput)) return msg.send("Duh! That prefix is not in your list.");

    prefix.splice(prefix.indexOf(prefixInput), 1);

    await msg.author.update({ prefix });

    return msg.send(`Berhasil removed the prefix \`${prefixInput}\` from your prefix list.`);
  }
}
