...
class Starboard extends Command {
  ...
  async run(msg, [action, amount]) {
    if(!action) return msg.send("Duh! Specify one of `enable #channel`, `disable` or `limit <amount>`");

    if(action === "disable") {
      await msg.guild.update({ starboard: { channel: null } });
      return msg.send("Berhasil disabled the server starboard.");
    }
    ...
    if(action === "enable") {
      ...
      return msg.send(`Berhasil enabled the server starboard for the channel ${channel}`);
    }

    if(action === "limit") {
      amount = this.verifyInt(amount);
      if(amount < 1) return msg.send("Duh! Limit cannot be less than 1");
      if(amount > msg.guild.memberCount) return msg.send("Duh! Limit cannot be more than the amount of members in the server.");
      await msg.guild.update({ starboard: { limit: amount } });
      return msg.send(`Berhasil updated the starboard star limit to ${amount}`);
    }

    return msg.send("Duh! Invalid action. Specify one of `enable #channel`, `disable` or `limit <amount>`");
  }
}
