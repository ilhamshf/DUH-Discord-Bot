...
class Pokemon extends Command {
  ...
  async run(msg) {
    ...
    if (!attempts || !attempts.size) {
      ...
      return msg.send(`Duh! You took too long to answer. It was ${pokemon.name}.`);
    }
    ...
    if (sent.deletable) await sent.delete();
    return msg.send(`Duh! You answered incorrectly, It was **${this.client.utils.toProperCase(pokemon.name)}.**`);
  }
}
