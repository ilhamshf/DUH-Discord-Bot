...
class Store extends Command {
  constructor(...args) {
    super(...args, {
      ...
      extendedHelp: "Examples:\n  **d!store add 1000 VIP** (sell VIP role for ¥1000)\n  **d!store add 0 Cool Guy** (Sell the Cool Guy role for free)\n  **d!store buy VIP** (buy the role.)\n  **d!store sell VIP** (sell and remove the role for a 50% refund)",
  ...
  async sell(msg, args) {
    ...
    if(!msg.member.roles.cache.has(role.id)) return msg.send("Duh! You don't have that role!");

    const store = this.client.settings.store.get(role.id);
    if(!store) return msg.send("Duh! That role is not for sale!");
    ...
    return msg.send(`Berhasil sold the role **${role.name}** for **¥${refund.toLocaleString()}** refund.`);
  }
  ...
  async buy(msg, args) {
    ...
    if(msg.member.roles.cache.has(role.id)) return msg.send("Duh! You already have that role.");
    ...
    if(msg.member.points < price)
      return msg.send(`Duh! You only have **¥${msg.member.points.toLocaleString()}**, but the role costs: **¥${price.toLocaleString()}**`);
    ...
    return msg.send(`Berhasil bought the role **${role.name}** for **¥${price.toLocaleString()}**`);
  }

  async delete(msg, args) {
    if(!msg.member.permissions.has("MANAGE_GUILD"))
      return msg.send("Duh! You need the `Manage Server` permissions to delete roles from the store.");
    ...
    return msg.send(`Berhasil removed the role **${role.name}** from the store.`);
  }

  async add(msg, [price, ...args]) {
    // Check for permissions.
    if(!msg.member.permissions.has("MANAGE_GUILD"))
      return msg.send("Duh! You need the `Manage Server` permissions to add roles to the store.");
    ...
    if(price >= Number.MAX_SAFE_INTEGER) return msg.send("Duh! That price is too high.");
