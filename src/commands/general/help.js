  ...
  async run(msg, [command]) {
    if(command) {
      ...
      if(cmd.nsfw && (msg.guild && !msg.channel.nsfw))
        return msg.send("Duh! You can't view details of that command in a non NSFW channel.");
      ...
      return msg.send(this.client.embed(this.client.user)
        ...
        .setDescription([
          ...
          `**Usage:** ${msg.guild ? msg.guild.prefix : "d!"}${cmd.usage}`,
          `**Cost:** ${cost}`,
    ...
    const embed = this.client.embed(this.client.user)
      ...
      .setDescription("Silahkan bergabung [Komunitas Kami](https://discord.gg/C7kgHCRYMW)")
      .setFooter("Untuk melihat informasi lebih jelas silahkan ketik d!help <command>");
