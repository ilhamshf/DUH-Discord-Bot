...
class Reputation extends Command {
  ...
  async run(msg, [user]) {
    ...
    if(user.bot) return msg.send("Duh! Bots cannot earn reputation points.");
    if(user.id === msg.author.id) return msg.send("Duh! You cannot give a reputation point to yourself.");
    if(msg.author.settings.repcooldown && (Date.now() < msg.author.settings.repcooldown))
      return msg.send(`Duh! You can give another reputation point in **${this.client.utils.getDuration(msg.author.settings.repcooldown - Date.now())}**`);
    ...
    return msg.send(`Berhasil gave a reputation point to ${user}`);
  }
}
